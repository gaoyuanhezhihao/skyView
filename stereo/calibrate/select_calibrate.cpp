...
int main(int argc, const char ** argv) {
    ...
    const int cols_sky_im = configs["cols_sky_im"];
    const int rows_sky_im = configs["rows_sky_im"];
    ...
    const int sample_id = get_param("sample_id");
    Sample rst;
    rst.rgb = imread(src_dir + to_string(sample_id) + ".jpg");
    Mat gray;
    ...
    int * p_sample_ids = new int[rst.pts.size()];
    for (size_t i = 0; i< rst.pts.size(); ++i) {
        p_sample_ids[i] = i;
    }
    Mat warped_img = calibrate(H_mat_path, rst.rgb, pt2ptf(rst.pts), floor_pts);
    imwrite(dst_dir+"skyView.jpg", warped_img);
    imshow("skyView", warped_img);
    waitKey(0);
}
